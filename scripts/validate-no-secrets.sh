...
if git grep -nEI '(BEGIN (RSA|EC|OPENSSH) PRIVATE KEY|gh[opsu]_[A-Za-z0-9]{20,}|postgresql[^ ]*://[^:]+:[^@]+@)' -- ':!scripts/validate-no-secrets.sh' ':!scripts/generate-secrets.sh' ':!.env.example'; then
